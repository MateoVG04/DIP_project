...
class HuMomentClass:
    def __init__(self):
        ...
        self.reference_area_image_2 = None
        self.reference_hu_2 = None
        self.reference_hu_1 = None
        self.reference_image_2 = None
        ...
        for contour in contours:
            ...
            if area > 500:
                ...
                if self.compare_hu_moments(hu_moments, self.reference_hu_1, threshold=0.5):
                    # Compare area size
                    print(area)
                    if abs(self.reference_area_image_1 - area) < 500:
                        x, y, w, h = cv2.boundingRect(contour)
                        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                # Compare hu moments with second reference image
                if self.compare_hu_moments(hu_moments, self.reference_hu_2, threshold=0.5):
                    # Compare area size
                    print(area)
                    if abs(self.reference_area_image_2 - area) < 500:
                        x, y, w, h = cv2.boundingRect(contour)
                        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        return frame
    ...
    def process_frames_and_save(self, input_npy, output_video_path, reference_image_1, reference_image_2):
        """
        ...
        """
        self.reference_image_1 = reference_image_1
        self.reference_image_2 = reference_image_2
        ...
        reference_gray_1 = cv2.cvtColor(reference_image_1, cv2.COLOR_BGR2GRAY)
        reference_gray_2 = cv2.cvtColor(reference_image_2, cv2.COLOR_BGR2GRAY)
        _, reference_binary_1 = cv2.threshold(reference_gray_1, 128, 255, cv2.THRESH_BINARY)
        _, reference_binary_2 = cv2.threshold(reference_gray_2, 128, 255, cv2.THRESH_BINARY)
        self.reference_hu_1 = self.calculate_hu_moments(reference_binary_1)
        self.reference_hu_2 = self.calculate_hu_moments(reference_binary_2)
        # Calculate area of reference image
        self.reference_area_image_1 = cv2.contourArea(cv2.findContours(cv2.cvtColor(self.reference_image_1, cv2.COLOR_BGR2GRAY), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0][0])
        self.reference_area_image_2 = cv2.contourArea(self.reference_image_2)
        print(self.reference_area_image_1)
        print(self.reference_area_image_2)
